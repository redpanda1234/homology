...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=2] % Syntax: [#1] optional arguments, (#2) center point, (#3:#4:#5) start angle, stop angle, radius; #6 secondary radius
    ...
    % Left Hole
    \draw (-\lholetx, \lholety) to [bend left] (\lholetx, \lholety);
    \draw (-\lholebx, \lholeby)  to [bend right] (\lholebx, \lholeby);

    % Right Hole
    \draw (\rholetxl, \rholety) to [bend left] (\rholetxr, \rholety);
    \draw (\rholebxl, \rholeby)  to [bend right] (\rholebxr, \rholeby);
    ...
    % \draw (\rtorusx, \rtorusy) arc (\rts:\rtf:\rx);
...
% \draw[red,dash pattern= on 3pt off 5pt,dash phase=4pt] (0,0) |- (1,1) to[out=0,in=90] (2,0);
    ...
    % Get paths for top and bottom connector segments
    \def\ptconn\path[name path=tconn] (\lconnx, \lconny) to[out=-\connout, in=\connin] (\rconnx, \rconny);
    \def\pbconn\path[name path=bconn] (\lconnx, -\lconny) to[out=-\connout, in=\connin] (\rconnx, -\rconny);

    % \draw[red,dash pattern=on 3pt off 5pt] (\lconnx,\lconny) to[out=-\connout,in=-\connin] (\rconnx,\rconny);
    % \draw[blue,dash pattern=on 3pt off 5pt, dash phase=4pt] (\lconnx,\lconny) to[out=-\connout,in=-\connin] (\rconnx,\rconny);

    % Bottom part of connecting segment
    \draw \ptconn;

    % I can't believe how un-robust the calc library is
    \pgfmathsetmacro{\cutoffset}{.5};
    \pgfmathsetmacro{\rcutoffset}{2+\cutoffset};
    \pgfmathsetmacro{\lcutoffset}{2-\cutoffset};
    \pgfmathsetmacro{\cutheight}{.682};

    % Right cut. use calc coordinate addition to avoid yet more
    % pgfmathmacro definitions
    \draw[dashed] ($(l\cutoffset, 0) - (0, \cutheight)$) arc (270:90:.2 and \cutheight);
    % \draw[dashed] ($(2,0)+(\cutoffset, -\cutheight)$) arc (270:90:.2 and \cutheight);
    \draw[] ($(2,0)+(\cutoffset, -\cutheight)$) arc (-90:90:.2 and \cutheight);

    % Left cut
    \draw[dashed] ($(2,0)+(-\cutoffset,-\cutheight)$) arc (270:90:.2 and \cutheight);
    \draw[] ($(2,0)+(-\cutoffset,-\cutheight)$) arc (-90:90:.2 and \cutheight);

    \path[name intersections={of=ltorus and tconn,by=E}];
    \node[dot=E]  at (E) {};
  \end{tikzpicture}
  \caption{torus}
\end{figure}
