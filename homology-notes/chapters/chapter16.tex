...
\begin{solution}
  ...
  \phi_2 = \set{\mb 0}$. Hence, $\phi_2$ is one-to-one (and in fact,
  similarly for all of the $\phi_n$). We will use this later.
  ...
  H_0(\TT^2) \cong \zmod{2}$. In fact, the same results hold for
  $k=1$. We summarize these results in the diagram below:
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}
      \pgfmathsetmacro{\xmin}{-6};
  ...
  % Now, note that
  % $\msf H_2(A) \cong \zmod{2} \cong \msf H_2(B)$. Hence $\msf H_2(A)
  % \oplus \msf H_2(B) \cong \zmod{2}\times \zmod{2}$. It follows that
  % $\ker \psi_2 = \set{(0,0), (1,1)}$. Since $\ker \psi_2 = \im
  % \phi_2$, this implies $\msf H_2(A \cap B) \cong \zmod{2}$.

  Since

  % then $\msf H_1(A \cap B) \cong \zmod{2} \times \zmod{2}$
  % (in each component, we either have a trivial cycle or a loop), and
  % $\msf H_0(A \cap B) \cong \zmod{2} \times \zmod{2}$ (Theorem 16.8).
  % This gives us
  % \begin{figure}[H]
  %   \centering
  %   \begin{tikzpicture}
  ...
  %     \node (2acb) at (-4,2) {$\msf H_2(A \cap B)$};
  %     \node (2apb) at (0,2) {$\zmod{2}\times \zmod{2}$};
  %     \node (2k) at (4,2) {$\msf H_2(K)$};
  ...
  %     \draw[->] (0apb) -- (0k) node[midway, above] {$\psi_0$};
  %   \end{tikzpicture}
  %   \caption{Mayer Vie-torus}
  % \end{figure}

  % since we know what $\phi_2$ , $\psi_2$ are, we can calculate $\msf
  % H_2(A \cap B)$, $\msf H_2(K)$ now. Observe that $\ker \psi_2 =
  % \set{(0,0), (1,1)} = \im \phi_2$. Thus, $\msf H_2(A \cap B)= $
\end{solution}
