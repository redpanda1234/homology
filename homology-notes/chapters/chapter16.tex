...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=2] % Syntax: [#1] optional arguments, (#2) center point, (#3:#4:#5) start angle, stop angle, radius; #6 secondary radius
    ...
    \pgfmathsetmacro{\lholety}{.03};

    % Bottom half of left hole arc
    \pgfmathsetmacro{\lholebx}{.75};
    \pgfmathsetmacro{\lholeby}{.09};

    % Parameters for drawing top half of right hole arc
    \pgfmathsetmacro{\rholetxl}{-.6+\rtorusx};
    \pgfmathsetmacro{\rholetxr}{.6+\rtorusx};
    \pgfmathsetmacro{\rholety}{.03+\rtorusy};

    % Bottom half of right hole arc
    \pgfmathsetmacro{\rholebxl}{-.75+\rtorusx};
    \pgfmathsetmacro{\rholebxr}{.75+\rtorusx};
    \pgfmathsetmacro{\rholeby}{.09+\rtorusy};
    ...
    % Left Hole
    \draw[\ltoruscolor] (-\lholetx, \lholety) to [bend left] (\lholetx, \lholety);
    \draw[\ltoruscolor] (-\lholebx, \lholeby)  to [bend right] (\lholebx, \lholeby);

    % Right Hole
    \draw[\rtoruscolor] (\rholetxl, \rholety) to [bend left] (\rholetxr, \rholety);
    \draw[\rtoruscolor] (\rholebxl, \rholeby)  to [bend right] (\rholebxr, \rholeby);
...
\begin{problem}[16.33]
  ...
  Equivalently put: let $\alpha \in \msf Z_k(A)$, and $\beta \in \msf
  Z_k(B)$. Suppose that
  \[
    [\alpha]_K = [\beta]_K.
  \]
  ...
  Or, show that if $\alpha - \beta = 0 \in \msf H_k(K)$, then $\exists
  c \in \msf Z_k(A \cap B)$ such that $(\alpha, \beta) = (c,c) \in
  \msf H_k(A) \oplus \msf H_k(B)$.
\end{problem}
\begin{solution}
  Define the homomorphisms $\pi_A : \msf Z_k(K) \to \msf Z_k(A)$,
  $\pi_B : \msf Z_k(K) \to \msf Z_k(B)$ as follows:
  \[
    \pi_A(\sigma) =
    \begin{cases}
    ...
    \end{cases}
  \]
  Observe that the following diagram commutes:\footnote{Ok,
    technically $\dom \pi_A = \msf Z_k(K) \neq \msf Z_k(B)$, but you
    could throw an inclusion map in there if you so pleased.}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}
  ...
  \end{figure}
  And also note that $\pi_A$, $\pi_B$ are idempotent. Now, since
  $[\alpha]_K = [\beta]_K$, there exists $c \in \msf B_k(K)$ such
  that $\alpha + \beta = c$. Note that
  \begin{align*}
    \partial \pn{(\pi_A \circ \pi_B)(c)}
    &= \pn{\pi_A \circ \pi_B}(\partial c) \\
    &= (\pi_A \circ \pi_B)(\partial (\alpha + \beta)) \\
    &= (\pi_A \circ \pi_B)
% \partial c
%     &=
%     &= (\pi_A \circ \pi_B)(\partial c') \\
%     &= (\pi_A \circ \pi_B)(\partial (\alpha + \beta)) \\
%     &= (\pi_A \circ \pi_B)(\mb 0) \\
%     &= \mb 0
  \end{align*}
  Hence $c \in \msf Z_{k}(A \cap B)$. Now,
  \begin{align*}
    \pi_A(\alpha - c)
    &= \pi_A(\pi_A(\alpha + \beta) - \pi_A(\beta) - c)
    % \pi_A(\alpha + \beta)
    % &= \pi_A(\alpha + (\pi_A(\beta) + \pi_B(\beta) - c)) \\
    % &= \alpha + \pi_A(\beta) + \pi_A(\pi_B(\beta)) - c \\
    % &= \alpha - c
  \end{align*}
  Hence, there exists $c_A \in \msf B_k(A)$, $c_B \in \msf B_k(B)$
  such that
  \begin{align*}
    \alpha
    &=
...
\begin{solution}
  \begin{enumerate}[label=(\arabic*)]
    ...
    \item WTS $\partial \alpha = \partial \beta \in \msf Z_{n-1}(A
      ...
      \end{align*}
      which are $n-1$ boundaries (and hence $n$-cycles).
    \item Suppose $z = \alpha' + \beta'$ is a sum of the form given.
      % Then
      % \begin{align*}
      %   c' + c
      %   &= \partial(\alpha') + \partial(\alpha) \\
      %   &= \partial(\alpha' + \alpha) \in \msf B_{n-1}(A \cap B)
      % \end{align*}
...
\begin{solution}
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \item Let $\psi(\sigma, \tau)$ be given by
      \[
        \psi(\sigma,\tau) = \sigma - \tau.
      \]
      It is straightforward to verify this is a well-defined
      homomorphism.
    \item Let $\delta(\sigma)$ be given by
      \[
        \delta(\upsilon) = \partial(\pi_A \circ \pi_B(\upsilon)).
      \]
      All the maps in this definition are linear on simplices.
      Homomorphism follows.
  \end{enumerate}
  Now, we show the composition things.
  \begin{enumerate}
    \item $\psi \circ \phi = 0$ is trivial
    \item Note
      \begin{align*}
        \delta(\psi(\alpha, \beta))
        &= \partial (\pi_A \circ \pi_B (\alpha - \beta)) \\
        &=
      \end{align*}
  \end{enumerate}
\end{solution}
