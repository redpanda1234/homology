...
\begin{enumerate}
  ...
  \item Undefined term. Section 16.4, Exercise 16.32: ``Can a {\color{red}
    ...
    Are we looking at $n$-cycles? Or $n$-chains? Furthermore: there are two
    somewhat-reasonable ways to parse the wording of this question. For the
    following, let $\iota : A\cap B \into A$, and let $\pi : A \onto A \cap A$.
    In the question, we define some $\sigma \in A \cap B$, but it's not
    completely clear based on the context whether we're being asked to consider
    \begin{enumerate}[label=(\arabic*)]
      \item Whether $\iota(\sigma)$ can be non-trivial in $A$, or
      \item Whether there exists non-trivial $\tau \in A$ such that
        $\pi(\sigma) = \iota$.
    \end{enumerate}
  % \item Ambiguous wording. Section 16.4, exercise 16.32: ``Note that a cycle in ''
